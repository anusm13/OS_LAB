...
int main() {
    ...
    for(int i = 0; i < n; i++) {
        struct process p;
        printf("\nProcess %d\n", i + 1);
        printf("Enter Arrival Time Burst Time Type(1-System,0-User): ");
        scanf("%d %d %d", &p.atime, &p.btime, &p.type);
        p.p_id = i + 1;
        if(p.type == 1)
            sys[sys_n++] = p;
        else
    ...
    for(int i = 0; i < k; i++) {
        printf("%d\t%s\t%d\t%d\t%d\t%d\t%d\n",all[i].p_id,(all[i].type == 1) ? "System" : "User",all[i].atime,all[i].btime,all[i].ctime,all[i].tat,all[i].wtime);
